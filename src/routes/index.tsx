...
function ErrorText({ children }: { children?: string }) { return children ? <p className="mt-1 text-xs text-gold-soft">{children}</p> : null; }
function Field({ label, name, type="text", placeholder, error }: { label:string; name:string; type?:string; placeholder:string; error?:string }) { return <div><label className="mb-2 block text-xs text-primary-foreground/65" htmlFor={name}>{label}</label><input id={name} name={name} type={type} maxLength={255} placeholder={placeholder} className="h-12 w-full rounded-none border border-primary-foreground/20 bg-primary-foreground/[0.04] px-4 text-sm text-primary-foreground outline-none placeholder:text-primary-foreground/30 focus:border-gold"/><ErrorText>{error}</ErrorText></div>; }
function SelectField({ label, name, options, error }: { label:string; name:string; options:string[]; error?:string }) { return <div><label className="mb-2 block text-xs text-primary-foreground/65" htmlFor={name}>{label}</label><select id={name} name={name} defaultValue="" className="h-12 w-full rounded-none border border-primary-foreground/20 bg-forest px-4 text-sm text-primary-foreground outline-none focus:border-gold"><option value="" disabled>Select one</option>{options.map(o => <option key={o} value={o}>{o}</option>)}</select><ErrorText>{error}</ErrorText></div>; }